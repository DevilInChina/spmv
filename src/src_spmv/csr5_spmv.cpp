...
void csr5HandleDestory(spmv_Handle_t handle) {
    if (handle) {
        if (handle->extraHandle && handle->spmvMethod == Method_CSR5SPMV) {
            anonymouslibHandle<int, unsigned int, double> &A = *((anonymouslibHandle<int, unsigned int, double> *) handle->extraHandle);
            A.destroy();
...
void csr5Spmv_get_handle_Selected(spmv_Handle_t handle,
    ...
    //printf("begin handle\n");
    handle->extraHandle = malloc(sizeof(anonymouslibHandle<int, unsigned int, double>));
    *((anonymouslibHandle<int, unsigned int, double> *) handle->extraHandle) = anonymouslibHandle<int, unsigned int, double>(
            m, n);
    anonymouslibHandle<int, unsigned int, double> &A = *((anonymouslibHandle<int, unsigned int, double> *) handle->extraHandle);
