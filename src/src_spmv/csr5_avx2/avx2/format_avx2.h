...
void generate_partition_descriptor_s2_kernel(const uiT *d_partition_pointer,
    ...
    int *s_segn_scan_all = (int *) _mm_malloc(2 * ANONYMOUSLIB_CSR5_OMEGA * sizeof(int) * num_thread,
                                              ANONYMOUSLIB_X86_CACHELINE);
    int *s_present_all = (int *) _mm_malloc(2 * ANONYMOUSLIB_CSR5_OMEGA * sizeof(int) * num_thread,
                                            ANONYMOUSLIB_X86_CACHELINE);
                                 ...
                                 const bool R2C) // R2C==true means CSR->CSR5, otherwise CSR5->CSR
{
    ...
    int num_thread = omp_get_max_threads();
    T *s_data_all = (T *) _mm_malloc(sigma * ANONYMOUSLIB_CSR5_OMEGA * sizeof(T) * num_thread,
                                     ANONYMOUSLIB_X86_CACHELINE);
